''' File to show how the chamber roughly behaves under different design parameters
    Based on simple Nusselt number, relations, not taking into account one-sided heating, assuming
    heater area is where energy is lost

    Basically, kinda bullshitty model that should predict qualitative behaviour, but are the numbers right?
'''
...
T_chamber = np.array([600,700,800, 900]) # [K] Desired exit temperature
...
### Calculate the tihngs that vary both with T_chamber and w_channel
it_T = np.nditer(T_chamber,flags=['c_index']) # [K] To iterate over all T_chamber values
#it_w = np.nditer(w_channel, flags=['c_index']) # [m] To iterate over channel values

# Storing results
Re_bulk =np.zeros((T_chamber.shape[0], w_channel.shape[0])) # [-] Bulk Reynolds number
...
for T in it_T:
    ...
    T_bulk = (T_inlet + T) / 2 # [K] Bulk temperature as reference
    ...
    delta_h = ideal_enthalpy_change(T_inlet=T_inlet, p_inlet=p_inlet, T_outlet=float(T), p_outlet=p_inlet, fp=fp) # [J/kg] Specific enthalpy change of the channel
    Q_dot = required_power(m_dot=m_dot, delta_h=delta_h) # [W] Power that must go into the flow to achieve delta_h 
    A_heater[it_T.index,:] = required_heater_area(Q_dot=Q_dot, h_conv=h_conv[it_T.index,:], T_wall=T_wall, T_ref=T_bulk) # [m^2] Required area to deliver power Q_dot
    ...
    P_radiation_loss[it_T.index,:] = radiation_loss(T=T_wall, A=A_heater[it_T.index,:], emmisivity=emmisivity) # [W] Radiation from single-sided heater surface
...
fig0.suptitle("Flow and heat transfer parameters for $T_{{wall}} = {:4.0f}$ K & $\dot{{m}}= {:3.1f}$ mg/s".format(T_wall, m_dot*1e6))
fig0.tight_layout()
axs0[0].legend(title="Chamber temp. $T_c$:", fontsize="small")
## Second set of subplots
...
# Make layout tight, add title and legend
fig1.suptitle("Channel length and power consumption for $T_{{wall}} = {:4.0f}$ K & $\dot{{m}}= {:3.1f}$ mg/s".format(T_wall, m_dot*1e6))
fig1.tight_layout()
axs1[1].legend(title="Chamber temp. $T_c$:", fontsize="small")
